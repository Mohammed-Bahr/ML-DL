...
# Process each line: remove the leading line number and spaces
commands = []
for line in lines:
    line = line.strip()
    if not line:
    ...
    # Split by whitespace
    parts = line.split()
    # If the first part is a digit, remove it
    if parts[0].isdigit():
...
# Prepare label encoder for first words (to be used in feature extraction)
all_first_words = [cmd.split()[0] if cmd.split() else '' for cmd in commands]
first_word_encoder = LabelEncoder()
first_word_encoder.fit(all_first_words)

# Feature extraction function (returns a dictionary with numeric features)
def extract_features(cmd):
    ...
    if first_word in first_word_encoder.classes_:
        features['first_word_encoded'] = first_word_encoder.transform([first_word])[0]
    else:
        features['first_word_encoded'] = -1  # unknown
...
print(f"Unique first words (first 10): {list(set(next_first_words))[:10]}")
...
if len(label_encoder.classes_) <= 10:
    ...
    # Show the top 5 most frequent classes in y
    from collections import Counter
    counter = Counter(y)
    print("Top 5 most frequent first words in y:", counter.most_common(5))
